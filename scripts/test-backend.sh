...
# Tests all API endpoints with optional domain parameter
###############################################################################
...
DOMAIN=${1:-"harmonyweb-2.vercel.app"}

echo "🧪 Testing Backend APIs on $DOMAIN"
echo "=========================================================="
echo ""
...
test_api() {
  ...
  if [ "$response" = "204" ] || [ "$response" = "200" ]; then
    ...
    # Check CORS headers
    cors=$(curl -s -X OPTIONS "https://${DOMAIN}${endpoint}" \
