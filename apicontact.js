 ...
 *
 * Same env vars as appointments.js
 */

const RATE_WINDOW_MS = 10 * 60 * 1000; // 10 min
const RATE_MAX = 10; // per IP per window for contact
...
function spamScore(payload) {
  ...
  if (linkHits >= 2) score += 4;     // multiple links = likely spam
  if (linkHits === 1) score += 1;    // one link: mild suspicion

  // very spammy keywords (kept narrow to reduce false positives)
  const bad = [
    "viagra", "casino", "porn", "escort", "forex", "crypto investment", "telegram me",
    "whatsapp me", "guaranteed profit", "loan offer", "click here"
  ];
  for (const w of bad) {
    if (lower.includes(w)) score += 3;
  }

  // If message is extremely short, mildly suspicious
